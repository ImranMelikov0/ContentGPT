...
class TextGenerator:
    ...
    def load_data(self):
        """Loads and cleans text data from a PDF file"""
        try:
            # Open the PDF file using pdfplumber
            with pdfplumber.open(self.config['data_path']) as pdf:
                text = ""
                for page in pdf.pages:
                    text += page.extract_text()  # Extract text from each page

            # Split the extracted text into sentences or paragraphs
            texts = text.split('\n')  # Split by newlines, assuming each line is a separate text
            texts = [self.clean_text(text) for text in texts if text.strip() != ""]

            print("Data successfully loaded. Total number of texts:", len(texts))
            ...
            print("File path:", self.config['data_path'])
            return None
